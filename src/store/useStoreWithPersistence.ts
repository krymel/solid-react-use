 ...
 * ```
 */
export const useStoreWithPersistence = <T extends object>(
  defaultState: T,
  options: StorePersistencyOptions<T> = getDefaultOptions<T>(),
) => {
  if (!options.name) {
    console.warn(
      `useStoreWithPersistence(${JSON.stringify(defaultState, null, 2).substring(
        0,
        50,
      )} ...[truncated], \n ${JSON.stringify(options, null, 2)}) was called without providing a 'name'. Using name '${
